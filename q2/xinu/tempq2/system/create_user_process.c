...
pid32 create_user_process(void *funcaddr,uint32 ssize,char *name,uint32 nargs, ...){
	...
	prptr->prprio = priority;		/*assign prio less than system processes, higher than null proc.*/
	...
	prptr->sleept_dur=0;
	prptr->ticketFlag=0;
	prptr->process_entry_time=ctr1000;
...
{	
	...
	if(prptr->tickets==0)
		prptr->ticketFlag=1;
	//kprintf("completed set_tickets yaya    $$$$$$ \n");
